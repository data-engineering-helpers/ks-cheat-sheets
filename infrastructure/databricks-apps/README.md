...
* [Cheat Sheet \- Databricks Apps](#cheat-sheet---databricks-apps)
...
* The `nyctaxi` schema (also known as a database) contains the `trips` table,
  which has details about taxi rides in New York City (NYC)
...
* A Lakebase/Postgres compute instance needs to be setup. Lakebase instances
  ...
  with
  `https://company-dev.cloud.databricks.com/compute/database-instances`

* For this cheat sheet, it is assumed that the Lakebase instance is called
  `lakebase-poc`.
...
* YAML files and resources:
  [GitHub - Data Engineering Helpers - Databricks Apps cheat sheet](https://github.com/data-engineering-helpers/ks-cheat-sheets/blob/main/infrastructure/databricks-apps/)
  * Original files:
  [GitHub - Databricks DAB examples - App React with Lakebase](https://github.com/databricks-solutions/databricks-dab-examples/tree/main/knowledge-base/app-react-lakebase/)
...
* The second step is to setup the
  ...
  * Source table: `samples.nyctaxi.trips`, which may usually be browsed on
  `https://company-dev.cloud.databricks.com/explore/data/samples/nyctaxi/trips`
  * Target catalog, schema and table: `poc.apps.nyctaxis`, which may then be
  browsed on (once the synchronization pipeline has completed)
...
psql -h $PG_SVR -U nyctaxi -d nyctaxi \
  -c "\copy (select * from apps.nyctaxis limit 10) \
  to 'nyctaxi-sample.csv' delimiter ',' csv header;"
```
